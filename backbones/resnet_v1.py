...
def residual_unit(data, num_filter, stride, dim_match, name, bottle_neck=True, bn_mom=0.9, workspace=512):
    ...
    if bottle_neck:
        ...
        bn2 = mx.sym.BatchNorm(data=conv2, fix_gamma=False, eps=2e-5, momentum=bn_mom, use_global_stats=True, name='bn'+name+'_branch2b')
...
def residual_layer(data, units, filter_list, name_list, conv1=True, bottle_neck=True, bn_mom=0.9, workspace=512):
    
    # build con1
    if conv1:
        conv1 = mx.symbol.Convolution(name='conv1', data=data , num_filter=64, pad=(3,3), kernel=(7,7), stride=(2,2), no_bias=True)
        bn_conv1 = mx.symbol.BatchNorm(name='bn_conv1', data=conv1 , use_global_stats=True, eps=2e-5, fix_gamma=False)
    ...
    for i in range(num_stages):
        ...
        conv = residual_unit(conv, filter_list[i], (1 if i==0 else 2, 1 if i==0 else 2), False, name=name_list[i]+sub_name[0], bottle_neck=bottle_neck, workspace=workspace)
        for j in range(units[i]-1):
