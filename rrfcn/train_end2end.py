...
from core.loader import Sequence_AnchorLoader
...
def train_net(args, config, ctx):
    
    model_path = config.pretrained_model.path
    epoch = config.pretrained_model.pretrained_epoch
    ...
    feat_sym = sym.get_internals()['rpn_cls_score_1_output']
    ...
    # load training data
    train_data = Sequence_AnchorLoader(feat_sym, roidb, config, batch_size=input_batch_size, shuffle=config.TRAIN.SHUFFLE, ctx=ctx,
    ...
    if config.TRAIN.RESUME:
        ...
        arg_params, aux_params = load_param_from_path(model_path, convert=True)
    else:
        arg_params, aux_params = load_param_from_path(model_path, convert=True)
